...
zabbix_sender -z $zbx_server -i $zbx_data --tls-connect=psk --tls-psk-identity="`cat /etc/zabbix/zabbix_agentd.conf | grep Iden | sed 's/=/ /' | awk '{print $2}'`" --tls-psk-file=/etc/zabbix/zabbix_agentd.psk&> /dev/null
